...
const ClientDashboardLayout = ({ children }: ClientDashboardLayoutProps) => {
  const { logout, currentUser, clients } = useMyuze();
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-myuze-black text-myuze-white p-4">
      <div className="mb-8 text-center">
        {client?.logoUrl ? (
          <img src={client.logoUrl} alt={`${client.name} Logo`} className="h-16 mx-auto object-contain mb-2" />
        ) : (
          <img src="/logo/logomyuzew.png" alt="Myuze Logo" className="h-16 mx-auto mb-2" />
        )}
        <p className="text-lg font-semibold text-myuze-white">{client?.name || 'Painel do Cliente'}</p>
      </div>
      ...
      <div className="flex flex-col flex-grow">
        ...
        {isMobile && (
          <header className="bg-myuze-black p-4 flex items-center justify-between shadow-md">
            {client?.logoUrl ? (
              <img src={client.logoUrl} alt={`${client.name} Logo`} className="h-10 object-contain" />
            ) : (
