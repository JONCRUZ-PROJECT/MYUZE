...
const EditBoardDialog = ({ children, board }: EditBoardDialogProps) => {
  const { updateBoard, playlists, currentUser } = useMyuze();
  const [isOpen, setIsOpen] = useState(false);
  const [name, setName] = useState(board.name);
  const [location, setLocation] = useState(board.location);
  const [selectedPlaylistId, setSelectedPlaylistId] = useState(board.playlistId);
  const [playerUsername, setPlayerUsername] = useState(board.playerUsername);
  const [playerPassword, setPlayerPassword] = useState(board.playerPassword);
  ...
  // Reset form fields when dialog opens or board prop changes
  useEffect(() => {
    if (isOpen) {
      setName(board.name);
      setLocation(board.location);
      setSelectedPlaylistId(board.playlistId);
      setPlayerUsername(board.playerUsername);
      setPlayerPassword(board.playerPassword);
  ...
  const handleSubmit = () => {
    ...
    updateBoard(board.id, updatedBoardData);
    setIsOpen(false);
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-myuze-black text-myuze-white border-myuze-purple">
        <DialogHeader>
          <DialogTitle className="text-myuze-white">Editar Quadro</DialogTitle>
          <DialogDescription className="text-gray-400">
            Modifique os detalhes do quadro e gerencie as credenciais do player.
          </DialogDescription>
        </DialogHeader>
